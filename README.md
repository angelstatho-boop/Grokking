...
## 1D DFT
To understand the embedding matrix and the activations of the MLP, neurons were extracted from the trained model and analyzed using a one-dimensional Discrete Fourier Transform. The graph of the Fourier spectra of the embedding matrix has spikes which means that the learned representations were not distributed evenly. It was concentrated at certain frequencies which produced a sparse frequency representation. This indicates that the network discovered a Fourier-based encoding of the modular addition task. The same pattern appeared MLP activations. This suggests that the model doesn’t memorize. These results provide evidence that the network has learned the underlying pattern and uses sparse frequency features.
